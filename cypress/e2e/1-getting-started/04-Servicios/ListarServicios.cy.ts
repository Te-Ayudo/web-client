...
describe('Flujo: Listar servicios', () => {
    ...
    beforeEach(() => {
        ...
        // Visitar la página
        cy.visit('/proveedor1/');
        cy.get('.swal2-container', { timeout: 10000 }) // Espera hasta 10s a que aparezca
            .should('be.visible') // Asegura que está visible
            .find('button.swal2-confirm') // Busca el botón de confirmación
            .click({ force: false }); // Haz clic en el botón de cerrar (no uses force)
    ...
    it('Debe Listar Servicios de Sucursal Seleccionada', () => {
        cy.visit(`/proveedor1/empresa/${BRANCH_ID}`);
