...
import { startTour, endTour, setCurrentPage } from "../store/tour";
...
const useTour = () => {
  ...
  const cleanupCurrentInstance = () => {
    ...
    try {
      const driverUINodes = document.querySelectorAll(
        '.driver-popover, .driver-overlay, .driver-backdrop, .driver-stage'
      );
      ...
      document.querySelectorAll('.driver-highlighted, .driver-active-element, .driver-position-relative')
    ...
    driverRef.current = driver({
      ...
      onClose: (params) => {
        if (!driverRef.current?._isCleanup) {
          ...
          console.log('Cleanup programático, manteniendo tour activo');
        }
        driverRef.current = null;
      },
      onDestroyed: () => {
        if (!driverRef.current?._isCleanup) {
          ...
          console.log('Destrucción programática, manteniendo tour activo');
        }
        driverRef.current = null;
      },
  ...
  const startSucursalesTour = async (sucursalesData = []) => {
    ...
    try {
      ...
      // Iterar por cada sucursal individualmente
      if (sucursalButtons.length > 0) {
        sucursalButtons.forEach((button, index) => {
          // Obtener el nombre de la sucursal desde los datos pasados o desde el DOM
          let sucursalName = `la sucursal ${index + 1}`;
          
          if (sucursalesData && sucursalesData[index]) {
            sucursalName = sucursalesData[index].name || `sucursal ${index + 1}`;
          } else {
            // Intentar extraer el nombre desde el atributo data-sucursal-name
            const nameFromAttr = button.getAttribute('data-sucursal-name');
            if (nameFromAttr) {
              sucursalName = nameFromAttr;
            }
          }

          // Paso 1: Botón de seleccionar sucursal
          const selectorSeleccionar = `[data-tour="seleccionar-sucursal"][data-index="${index}"]`;
          steps.push({
          ...
          // Paso 2: Botón de ubicación de la misma sucursal
          if (ubicacionButtons[index]) {
            const selectorUbicacion = `[data-tour="ver-ubicacion"][data-index="${index}"]`;
            steps.push({
    ...
    try {
      ...
      if (servicioItems.length > 0) {
        if (isServiciosPage) {
          ...
          // En la página de Empresa, mostrar todos los servicios como antes
          servicioItems.forEach((item, index) => {
            // Intentar obtener el nombre del servicio desde los datos pasados o desde el DOM
            let serviceName = "este servicio";
            
            if (serviciosData && serviciosData[index]) {
              serviceName = serviciosData[index].name || `servicio ${index + 1}`;
            } else {
              // Intentar extraer el nombre del servicio desde el atributo data-service-name
              const nameFromAttr = item.getAttribute('data-service-name');
              if (nameFromAttr) {
                serviceName = nameFromAttr;
              } else {
                // Como último recurso, buscar en el DOM
            ...
            // Usar un selector más específico basado en el índice o atributo data-index
            const selector = `[data-tour="servicio-item"][data-index="${index}"]`;
            ...
            steps.push({
              ...
              popover: {
                title: `${serviceName} 🛎️`,
                description: `Haz clic si quieres contratar el servicio de "${serviceName}". Podrás ver sus detalles y agregarlo a tu carrito de compras.`,
                position: index % 2 === 0 ? "top" : "bottom" // Alternar posición para mejor visualización
              }
            });
          });
        }
  ...
  const startServiceModalTour = async (serviceName = "este servicio") => {
    ...
    try {
      ...
      console.log('Elementos del modal encontrados:', {
        btnRestar: !!btnRestar,
        btnSumar: !!btnSumar,
        btnAñadirCarrito: !!btnAñadirCarrito
      });
      ...
      const conflictingElements = document.querySelectorAll('[data-tour="servicio-item"], [data-tour="buscador-servicio"]');
  ...
  const checkAndStartTour = (pageName, extraData = null) => {
    ...
    // NUEVA LÓGICA: Prevenir bucles infinitos
  ...
  const stopServicesTour = () => {
    ...
    document.querySelectorAll('.driver-popover, .driver-overlay, .driver-backdrop, .driver-stage')
      ...
      if (listaServicios) {
        steps.push({
          ...
          popover: {
            ...
            description: "Aquí puedes ver todos los servicios que has agregado a tu carrito. Puedes modificar las cantidades si es necesario.",
            position: "left"
          }
        });
      }
      ...
      if (btnSolicitar) {
        steps.push({
          ...
          popover: {
            ...
            description: "¡Perfecto! Cuando estés listo, haz clic aquí para proceder a programar tu cita y finalizar tu solicitud.",
            position: "top"
          }
        });
      }
      ...
      if (steps.length > 0) {
        ...
        driverObj.destroy = function() {
          try {
            ...
            tourExecutingRef.current = false; // Liberar la flag
            originalDestroy();
          } catch (destroyError) {
