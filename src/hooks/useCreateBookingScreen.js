...
export const useCreateBookingScreen = (selectedEmployee, selectedDate, selectedTime, formData, selectedAddress, updateFormData) => {
  ...
  const [paymentMethods, setPaymentMethods] = useState([{ label: "Efectivo", value: "Efectivo" }]);
  ...
  const onSubmit = (event) => {
    ...
    if (!selectedDate || !selectedTime || !formData.metodopago || booking.serviceCart.length === 0) {
      Swal.fire({
        icon: "error",
        title: "Error",
    ...
    setLoading(true);
    dispatch(startCreateBooking(bookingData, providerid, navigate));
  };
  ...
  useEffect(() => {
    ...
    const cardPaymentEnabledArray = booking.serviceCart?.map((e) => e.service?.cardPaymentEnabled);
