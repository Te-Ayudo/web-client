...
export const Lista = ({ servicio, showallicon = false, quantity = 0, orden = "", index = 0 }) => {
  ...
  return (
    <>
      <div
        onClick={onAddCart}
        className="flex justify-between items-center border-b border-slate-200 py-3 px-2 border-l-4 cursor-pointer  border-l-transparent mb-5"
      ...
      >
        <div className="w-2/3 bg-white flex flex-col space-y-2 p-3  inline-flex items-start space-x-2 text-wrap break-all ">
          ...
          <div className="text-slate-500">{textLonger}</div>
          <div className="text-slate-500">
        ...
        <div className="flex flex-col w-1/3 h-fit items-end">
          ...
          <div className="mt-2 float-end align-center">
            {showallicon ? (
              ...
              <button type="button" className="btn-icon" onClick={onAddCart}>
                <svg xmlns="http://www.w3.org/2000/svg" width="0.8em" height="0.8em" viewBox="0 0 24 24">
                  <path fill="currentColor" d="M21 13h-8v8h-2v-8H3v-2h8V3h2v8h8z" />
                </svg>
                <span className="sr-only">Icon description</span>
