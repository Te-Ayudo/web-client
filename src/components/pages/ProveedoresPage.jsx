...
export const ProveedoresPage = () => {
  ...
  useEffect(() => {
    const filtered = filteredByBranchAvailability(providers);
    setFiltered(filtered);
  }, [providers]);

  const filteredByBranchAvailability = (providers) =>
    selected.isInBranch
  ...
  return (
    <>
      <Main header={<Header />} footer={<Footer />}>
        <List>
          <Button className="btn-auto font-normal mb-5" disabled={true}>
            {" "}
            Sucursales{" "}
          </Button>
          <ul>
