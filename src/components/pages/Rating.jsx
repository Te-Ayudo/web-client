...
export const Rating = () => {
  ...
  const onSubmit = async (event) => {
    ...
    try {
      ...
      const messageError =
        error === "Ya se ha enviado la calificación."
          ? "Ya se ha enviado la calificación."
  ...
  return (
    <div className="flex flex-col justify-center items-center min-h-screen p-6 bg-gray-100">
      <div className="bg-white shadow-lg rounded-lg p-6 w-full max-w-md">
        ...
        <form type="POST" onSubmit={onSubmit} className="flex flex-col items-center gap-6">
          <div className="flex flex-col items-center text-center">
            <p className="text-md font-medium text-gray-700">Califica el servicio:</p>
            <div className="flex gap-2 p-2">
              {Array.from({ length: 5 }).map((_, index) => (
                <FiStar
                  ...
                  onClick={() => setFormValues((prev) => ({ ...prev, rating: index + 1 }))}
                />
              ))}
            </div>
          </div>
