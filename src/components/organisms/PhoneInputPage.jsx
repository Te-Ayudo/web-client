...
const PhoneInputPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    dispatch(
      startLoginWithWhatsapp({ phone, codePhone }, () => {
  ...
  return (
    <Main header={<Header />}>
      <Modal>
        <form onSubmit={handleSubmit} className="pt-10 pb-6 flex flex-col justify-between items-center text-center">
          <div className="w-full">
            <div className="mb-6 sm:mb-10">
              <h1 className="text-orange-500 font-bold text-3xl">Bienvenido a {providerid}</h1>
            </div>
            <h2 className="text-xl font-bold text-orange-500 mb-2">Ingresa tu número de teléfono</h2>
            <p className="text-sm text-gray-600 mb-3">
              Recibirás un código de 6 dígitos enviado a tu WhatsApp. Esto puede tardar 1 minuto
            </p>

            <div className="bg-white rounded-2xl py-4 w-full max-w-sm mx-auto">
              <label className="text-left text-sm font-semibold text-gray-800 block mb-2">Número de celular</label>
              <div className="flex gap-2">
                ...
                <Select defaultValue={codePhone} onValueChange={setCodePhone}>
                  ...
                  <SelectContent className="max-h-60">
                    {paises.map((p) => (
                      <SelectItem key={p.iso2} value={p.dial_code} className="flex items-center gap-2 py-2">
                        <CountryFlag countryCode={p.iso2} svg style={{ width: "1.5rem", height: "1.5rem" }} />
